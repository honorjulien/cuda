...
void bilateral_filter(Image *img) {
    ...
    // Définir les événements pour mesurer le temps
    ...
    printf("Temps d'exécution du noyau CUDA : %.4f millisecondes\n", elapsedTime);
}
...
int main(int argc, char *argv[]) {
    ...
    Image img;
    load_image(argv[1], &img); // Charge l'image d'entrée
    
    // Démarre le chronomètre pour le filtrage
    bilateral_filter(&img); // Applique le filtre bilatéral sur le GPU
    
    save_image(argv[2], &img); // Sauvegarde l'image filtrée
